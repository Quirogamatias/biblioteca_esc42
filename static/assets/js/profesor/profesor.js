    ...
    $.ajax({
        ...
        success: function(response){
            ...
            for(let i = 0;i < response.length;i++){
                ...
                fila += ' onclick = "abrir_modal_edicion(\'/libro/editar_profesor/' + response[i]['pk']+'/\');"> EDITAR </button>';
                fila += '<button type = "button" class = "btn btn-danger tableButton btn-sm"';
                fila += 'onclick = "abrir_modal_eliminacion(\'/libro/eliminar_profesor/'+ response[i]['pk']+'/\');"> ELIMINAR </button>';
                fila += '<button type = "button" class = "btn btn-info tableButton btn-sm"';
                fila += 'onclick="location.href=(\'/usuarios/cambiar_passwordP/'+ response[i]['pk']+'/\');"> cambiar contraseña </button> </td>';
                fila += '</tr>';
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            for(let i = 0;i < response.length;i++){
                ...
                fila += '<td>' + response[i]["fields"]['telefono'] + '</td>';                      
                fila += '<td> <button type = "button" class = "btn btn-primary btn-sm tableButton"';
                fila += ' onclick = "abrir_modal_edicion(\'/libro/editar_profesorP/' + response[i]['pk']+'/\');"> EDITAR </button></td>';
                fila += '</tr>';
...
function eliminar(pk){
	$.ajax({
		...
		url: '/libro/eliminar_profesor/'+pk+'/',
		type: 'post',
		success: function(response) {
            notificacionSuccess(response.mensaje);
