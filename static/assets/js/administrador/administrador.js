function listadoAdministradores(){
    $.ajax({
        url: "/libro/listar_administradores/",//se llama al prime nombre listar_administradores
        type: "get",
        dataType: "json",
        success: function(response){
            ...
            for(let i = 0;i < response.length;i++){
                ...
                fila += ' onclick = "abrir_modal_edicion(\'/libro/editar_administrador/' + response[i]['pk']+'/\');"> EDITAR </button>';
                fila += '<button type = "button" class = "btn btn-danger tableButton  btn-sm" ';
                fila += 'onclick = "abrir_modal_eliminacion(\'/libro/eliminar_administrador/' + response[i]['pk'] +'/\');"> ELIMINAR </buttton>';
                fila += '<button type = "button" class = "btn btn-info tableButton btn-sm"';
                fila += 'onclick="location.href=(\'/usuarios/cambiar_passwordAD/'+ response[i]['pk']+'/\');"> cambiar contraseña </button> </td>';
                fila += '</tr>';
...
function eliminar(pk){
	$.ajax({
		...
		url: '/libro/eliminar_administrador/'+pk+'/',
		type: 'post',
		success: function(response) {
            notificacionSuccess(response.mensaje);
			listadoAdministradores();
